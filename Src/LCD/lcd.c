...
void LCD_begin(LCD_TypeDef* lcd) {
  ...
  sendCommand(LCD_DISPLAYCONTROL | LCD_DISPLAYON | LCD_BLINKOFF | LCD_CURSOROFF);
  HAL_Delay(5);
...
//  if((lcd->currLine == 0) && (lcd->currCol == 19))
//    LCD_setCursor(lcd, 1, 0);
//  else if((lcd->currLine == 1) && (lcd->currCol == 19))
